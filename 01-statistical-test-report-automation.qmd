...
call_out_text <- case_when(
  test_outcome == 1 & relative_uplift_conversion_rate > 0 ~ "is large enough to declare a significant winner. There is evidence of a difference in performance between A and B.",
...
: Figure 1: Test summary {.hover tbl-colwidths="\[75, 50, 50, 75\]"}

Test parameters:

| Lift (%)                                                              |               Confidence Level               |   Test Type   |        P-value        |
|-----------------|:---------------:|:---------------:|:-------------------:|
| `r scales::percent(relative_uplift_conversion_rate, accuracy = 0.01)` | `r scales::percent(params$confidence_level)` | `r test_type` | `r round(p_value, 4)` |

: Figure 2: Test parameters {.hover tbl-colwidths="\[75, 50, 50, 75\]"}
